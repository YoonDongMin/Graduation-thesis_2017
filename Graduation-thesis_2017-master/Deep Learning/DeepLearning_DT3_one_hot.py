...
X = tf.placeholder(tf.float32, [None, 1503])
Y = tf.placeholder(tf.float32, [None, 49])

keep_prob = tf.placeholder(tf.float32)

W1 = tf.get_variable("W1", shape=[1503, 500],
                     initializer=tf.contrib.layers.xavier_initializer())
b1 = tf.Variable(tf.random_normal([500]))
L1 = tf.nn.relu(tf.matmul(X, W1) + b1)
L1 = tf.nn.dropout(L1, keep_prob=keep_prob)

W2 = tf.get_variable("W2", shape=[500, 49],
                     initializer=tf.contrib.layers.xavier_initializer())
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        one_hot_X = np.reshape(one_hot_X, (500, 1503))
